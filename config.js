...
const getConfig = (startId = 0) => ({
  ...
  sampleAppId: startId + 1,
  sampleVersionId: startId + 1, //sample app version id
  sampleCustomTagId: -1, //sample custom tag id, or -1 to create a new custom tag before adding
  sampleSecondaryVersionId: startId + 4,
  sampleArtifactId: startId + 10, // sample fpr id
  downloadFolder: "/downloads", //relative to root project, wil be created if non existant
  sampleUserId: startId + 4,
  sampleAuthEntities: [
    { id: startId + 12, isLdap: false },
    { id: startId + 9, isLdap: false },
    { id: startId + 6, isLdap: false },
  ],
  sampleAttributeDefinition: {
    name: "",
    description: "Created by SSC-js-sandbox",
    category: "ORGANIZATION",
    type: "TEXT",
    appEntityType: "PROJECT_VERSION",
  },
  sampleCustomTag: {
    name: "",
    description: "Created by SSC-js-sandbox",
  ...
  sampleAttributeValue: [
    { attributeDefinitionId: undefined, values: null, value: undefined },
  ],
...
module.exports = {
  ...
  loadEnv: () => {
    ...
    const config = getConfig(startId);
